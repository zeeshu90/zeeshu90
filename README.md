# Hi, I'm Shokeen Teeli 👋

**Vibe Coder · Full-Stack Developer · AI Trainer**  
Building real products with AI-assisted workflows — fast, functional, and production-ready.

---

## 🚀 What I Do

- **Vibe Coding** — I use GitHub Copilot & Claude AI daily to ship features in hours, not days
- **Web Scraping** — Python, BeautifulSoup4, Playwright for JS-heavy sites
- **Full-Stack Dev** — Next.js, React, Tailwind CSS, Supabase, Vercel
- **AI Training** — Designing and verifying problems for AI model training (Data Science & Coding domains)

---

## 🛠️ Tech Stack

![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=next.js&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=postgresql&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=flat&logo=supabase&logoColor=white)
![Tailwind](https://img.shields.io/badge/Tailwind-06B6D4?style=flat&logo=tailwindcss&logoColor=white)
![Playwright](https://img.shields.io/badge/Playwright-2EAD33?style=flat&logo=playwright&logoColor=white)
![GitHub Copilot](https://img.shields.io/badge/GitHub_Copilot-000000?style=flat&logo=github&logoColor=white)
![Claude](https://img.shields.io/badge/Claude_AI-D97757?style=flat&logo=anthropic&logoColor=white)

---

## 🏗️ Featured Project

### [JkssbPrep](https://jkssbprep.in) — Live Exam Prep Platform
> Serving JKSSB competitive exam aspirants across Jammu & Kashmir

- Built with **Next.js + Supabase + Vercel** — fully production deployed
- Razorpay payment integration with HMAC webhook verification
- SEO-optimized with structured data (FAQ, Course, Article schemas)
- Test series, practice questions, blog & student store
- Shipped rapidly using **AI-assisted vibe coding workflow**

---

## 📍 About Me

- 🌍 Based in **Jammu & Kashmir, India**
- 🎓 BSc (Expected 2027)
- 🤖 Currently doing freelancing.
- 📬 **xeeshannteeli@gmail.com**
- 🌐 [jkssbprep.in](https://jkssbprep.in)

---

*"Ship fast, verify carefully, improve continuously."*
